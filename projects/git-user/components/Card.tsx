...
const Card = () => {
	const { userAccount, setUserAccount, username } = useGitUser();
	const { data: user, isLoading, refetch } = useFetchUser(username);
  ...
  return (
	<main className={`${Styles.card}`}>
		<section className={`${Styles.header}`}>
			<Image className={`${Styles.header_img}`} width={100} height={100} src={ userAccount.avatar_url } alt="profile icon github" />
			<div className={`${Styles.header_text}`}>
				...
				<p className={`${Styles.header_text__joinDate}`}> 
					Joined { userAccount.created_date }
				</p>
			</div>
		...
		<section className={`${Styles.bio}`}>
			{userAccount.bio
				?<h3 className={`${Styles.bio_text}`}>{ userAccount.bio }</h3>
		...
		<section className={`${Styles.stats}`}>
			<div className={`${Styles.states_sub}`}>
				<p className={`${Styles.states_header}`}>Repos</p>
				<h3 className={`${Styles.stats_number}`}>{ userAccount.public_repos }</h3>
			</div>

			<div className={`${Styles.stats_sub}`}>
				<p className={`${Styles.stats_header}`}>
					Followers
				</p>
				<h3 className={`${Styles.stats_number}`}>{ userAccount.followers }</h3>
			</div>

			<div className={`${Styles.stats_sub}`}>
				<p className={`${Styles.stats_header}`}>Following</p>
				<h3 className={`${Styles.stats_number}`}>{ userAccount.following }</h3>
			</div>
		</section>
		...
		<section className={`${Styles.links}`}>
			...
			<svg height="20" width="14" xmlns="http://www.w3.org/2000/svg"><path d="M12.797 3.425C11.584 1.33 9.427.05 7.03.002a7.483 7.483 0 00-.308 0C4.325.05 2.17 1.33.955 3.425a6.963 6.963 0 00-.09 6.88l4.959 9.077.007.012c.218.38.609.606 1.045.606.437 0 .828-.226 1.046-.606l.007-.012 4.96-9.077a6.963 6.963 0 00-.092-6.88zm-5.92 5.638c-1.552 0-2.813-1.262-2.813-2.813s1.261-2.812 2.812-2.812S9.69 4.699 9.69 6.25 8.427 9.063 6.876 9.063z" fill="#4b6a9b"/></svg>
				{userAccount.location
					?<p className={`${Styles.links_text}`}>{ userAccount.location }</p>
					:<p className={`${Styles.links_text} ${Styles.lowLight}`}>Not Available</p>
				}
			</div>
			...
			<div className={`${Styles.links_item}`}>
				...
				{userAccount.twitter
					?<p className={`${Styles.links_text}`}>
						<a href={ userAccount.twitter } rel="noreferrer" target="_blank">{ userAccount.twitter }</a>
					</p>
					:<p className={`${Styles.links_text} ${Styles.lowLight}`}>Not Available</p>
				}
			...
			<div className={`${Styles.links_item}`}>
				...
				{userAccount.company
					?<p className={`${Styles.links_text}`}>{ userAccount.company }</p>
					:<p className={`${Styles.links_text} ${Styles.lowLight}`}>Not Available</p>
				}
			</div>
		</section>
